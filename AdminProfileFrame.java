...
public class AdminProfileFrame extends JFrame {
    ...
    public AdminProfileFrame(String username) {
    ...
    private void loadProfileData() {
        try {
            ...
            nameColumn = findExistingColumn(con, "admins", "name", "admin_name", "full_name");
            emailColumn = findExistingColumn(con, "admins", "email", "mail", "admin_email");
    ...
    private void saveProfileChanges() {
        try {
            ...
            if(setParts.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Name/Email columns not found in admins table.");
                con.close();
                return;
            }

            String sql = "UPDATE admins SET " + String.join(", ", setParts) + " WHERE username=?";
            PreparedStatement pst = con.prepareStatement(sql);
